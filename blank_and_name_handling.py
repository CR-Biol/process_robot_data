...
def process_well_input(inp):
    ...
    if len(inp_list) == 1:
        # Tries to parse the integer-only input variant.
        try:
            row = int(inp_list[0])
            cols = "ABCDEFGH"
            ...
            # In this case, only a single well but no full column is intended.
            pass
...
def is_valid_well_input(well_input_list):
    ...
    process_well_input().
    Will ignore one mistake: If the colon notation is used and the order is 
    incorrect, it will currently (v0.9.2) accept the input as valid. I.e.,
    'A3:A1' will be accepted."""
    valid_patterns = [
...
def get_wrappers(path_to_file, blank_wells, use_fixed_od_blank=False,
        ...
        for line_idx, line in enumerate(file):
            if line_idx in lines_od:
                ...
                for val_idx, val in enumerate(vals):
                    wrapper_od[constants.col_names[val_idx]].append(
                        float(val.replace(",", ".")))
            elif line_idx in lines_fu:
                # Finds lines that correspond to reporter data.
                vals = line.strip().split()
                for val_idx, val in enumerate(vals):
                    wrapper_fu[constants.col_names[val_idx]].append(
                        float(val.replace(",", ".")))
    ...
    def cap_negative_values_to_zero(list_of_numbers):
    ...
    for data_col in constants.data_names:
        ...
        wrapper_od[data_col] = [
            item for item in cap_negative_values_to_zero(wrapper_od[data_col])]
        wrapper_fu[data_col] = [
            item for item in cap_negative_values_to_zero(wrapper_fu[data_col])]
...
def sort_df(dataframe_or_csv_file):
    ...
    # Filters and sorts columns according to to_keep
