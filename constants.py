...
def blank_correct(value, blank_value):
    return cap_negative_number_to_zero(value - blank_value)


def cap_negative_number_to_zero(number):
    if number > 0:
        return number
    else:
        return 0
...
def parse_number(number_as_str):
    return float(number_as_str.replace(",", "."))
